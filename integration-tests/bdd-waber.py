...
def sign_up_as_passenger(name, email, password):
    ...
    return requests.post('/api/users', params={'type': 'passenger'},
...
def sign_up_as_driver(name, email, password, car_type):
    ...
    return requests.post('/api/users', params={'type': 'driver'},
                         data={'name': name, 'email': email, 'password': password})
...
def participate_valentines_day(driver):
    ...
    requests.post(f'/api/activities/valentinesDay/drivers/{driver.id}')
...
def update_driver_location(user, location, location_alias):
    ...
    requests.post(f'/api/users/{user.id}/location', params=location)
...
def update_passenger_location(user, location, location_alias):
    ...
    requests.post(f"/api/users/{passenger.id}/location", params=location)


def start_matching():
    print(f"乘客{passenger.name} 開始匹配...")
    # [POST] /api/users/${passenger_id}/match
    return requests.post(f"/api/users/{passenger.id}/match")
...
def wait_for_match(match_id):
    ...
    match = requests.get(f'http://localhost:8080/api/users/{passenger.id}/match/{match_id}')
    if match.completed:
        driver = None
...
def pick_up(match_id):
    ...
    requests.post(f"/api/users/{passenger.id}/match/{match_id}/trip")
...
def arrive(match_id, trip_id):
    ...
    requests.post(f'/api/users/{passenger.id}/match/{match_id}/trip/{trip_id}/arrive')
...
def pricing(match_id, trip_id):
    ...
    results = requests.get(f'/api/users/{passenger.id}/match/{match_id}/trip/{trip_id}/price')
